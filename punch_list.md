...
- [ ] **CASES-P2-2** — `Allclean` is only called from the suite runner (`non-regression_local.sh`), never from inside individual Allrun scripts. Confirm design intent.
...
<a id="code-feature-7"></a>
- [x] **CODE-FEATURE-7** — *Resolved 2026-06-12.* **PWR-rod fidelity + solver sprint** (all documented in CONTEXT.md §3.3/§4.5/§4.5bis/§4.8/§5/§6.1): (i) Fink (2000) UO2 k(T) as a symbolic card (`materials/fuel_thermal.py`); (ii) Robin convective coolant on the rod case; (iii) `fuel_swelling.solid_gas_densification` — solid + T-activated gaseous swelling + early-life densification behind the existing eigenstrain hook; (iv) **irradiation creep** ε̇ = B·φ·σ inside the Norton radial return (`creep_model.py`; exact no-op without `creep_irr_B`+`fast_flux` — both creep golds unchanged at machine precision); (v) **Barani 2019 fuel cracking** as `models/cracking_model.py` + spine validation, opt-in `cracking: isotropic`; (vi) gap-model fix: symbolic k(T) evaluated at the gap temperature for the Ross-Stoute harmonic mean (previously h_contact silently zeroed — the contact-cooling feedback was off for any case with symbolic fuel k); (vii) **Aitken Δ² relaxation** (`relax_aitken`, hot-reloadable; per-step ω restart + ‖ΔR‖ noise guard are load-bearing — without them one degenerate step poisons the multiplicative recursion); (viii) **gap-conductance under-relaxation** (`gap_conductance.relax`); (ix) **per-step form caching** in `_thermal_step`/`_mechanical_step` (~2 500 → ~109 form builds per rod run; validated PASS/PASS against 7 golds spanning thermal/gap/contact/SNES-creep paths); (x) per-segment `n_steps` list in `generate_power_history` (backward compatible); (xi) rod case redesigned: 20 kW/m, 1800 d, grid [8, 60, 40], MUMPS, mech stag_tol 5e-4.

<a id="cases-followup-9"></a>
- [ ] **CASES-FOLLOWUP-9** *(added 2026-06-12)* — Verification debt from CODE-FEATURE-7: (a) ~~an **irradiation-creep verification case**~~ — *done 2026-06-13*: `verification/fuel/creep_irradiation` (constant-stress bar, Norton made negligible via `creep_A0=1e-50`, linear in-pile term ε_cr = B·φ·σ·t; matches to 1.7e-13). (b) ~~a **cracking-model unit case**~~ — *done 2026-06-13*: `verification/fuel/cracking` (ramp LHR 0→40→20 kW/m, parses the `[cracking]` log and checks n, E_iso/E, ν_iso vs the Barani Eqs. 9/10/13 to ~1e-6, the paper headline n≈6.6 at 20 kW/m, and **irreversibility** — the down-ramp leaves n unchanged; the cracking log line precision was bumped to `.4f/.6f` to support the parse). (c) ~~re-bless the `regression/pwr_rod_2D` gold~~ — *done 2026-06-12* from the definitive run (105 min, summary PASS + closed-form burnup exact; note the case's `non-regression.py` needed the same int-or-list `n_steps` handling as `__main__`); (d) consider an **Aitken A/B case** (same rod, `relax_aitken` on/off) as solver-performance evidence for the paper.

<a id="code-p2-10"></a>
- [x] **CODE-P2-10** — *Resolved 2026-06-13.* **`creep_A0 = 0` (pure irradiation creep) crashed the predictor.** `update_creep_predictor` built `dolfinx.fem.Expression(ufl.as_ufl(self._creep_A(material, T)), ...)`; with `creep_A0 = 0` the term `A0·exp(-Q/RT)` folds to a meshless UFL constant and dolfinx raised "Could not extract MPI communicator for Expression" (it derives the comm from the expression's coefficients, and a bare constant has none). The symbolic creep *form* (`creep_increment`) was always fine — `Zero` terms just drop in a compiled form; only the predictor's *interpolation* Expression needed the mesh. **Fix:** `update_creep_predictor` now short-circuits to `Adt = np.zeros_like(sig)` when `creep_A0 == 0` instead of building the Expression. `verification/fuel/creep_irradiation` switched from the `1e-50` workaround to a clean `creep_A0: 0` (pure in-pile creep, ε_cr = B·φ·σ·t to 8e-14); Norton creep case unaffected (`A0 ≠ 0` takes the original branch).

<a id="code-p1-15"></a>
- [x] **CODE-P1-15** — *Resolved 2026-06-13.* **Output stress frozen at virgin stiffness for cracking runs (writer baked step-0 elastic constants).** Symptom: in `regression/pwr_rod_2D` the written fuel `Stress` field was physically impossible — `σ_rr` tensile in the outer pellet and ~+700 MPa at the traction-free (−2 MPa fill-gas) surface, magnitudes ~virgin-stiffness (centre −1333 MPa) despite cracking softening E to ~0.11·E. Root cause: `utils/writer.py::OutputWriter` compiles every field Expression **once at construction** (the deliberate no-JIT-in-loop optimisation), baking `material["lmbda"]`/`["G"]` as **float literals**. The writer is built (`__main__.py:263`) right after the step-0 `get_results()`, which ran at `lhr = lhrs[0] = 0.0` → **uncracked/virgin** constants. The solver form rebuilds per step so the *displacement* used the correct soft constants, but the *written* stress was `ℂ_virgin:(ε(u_soft) − ε*_virgin)` — inconsistent with `u`, violating equilibrium and the surface BC. The eigenstress error dominates because cracking drops the bulk modulus K = E/(3(1−2ν)) by ~29× (ν 0.345 → 0.003). **Why it hid:** the only cracking case is `pwr_rod_2D` (XDMF, no per-radius stress check); `verification/fuel/coaxial_contact` passes because it has **no cracking** → constants never change → the step-0 baked Expression equals every step's value. It also has **uniform** pellet temperature, so it never exercised the thermal-gradient stress path either. **Diagnosis tooling added (kept):** `coaxial_contact/non-regression.py` now has `plot_stress_profiles()` overlaying `σ_rr`/`σ_θθ` on the closed-form Lamé interference field (pellet −p uniform; clad thick-wall) — the controlled check that proved continuity and isolated the defect to cracking. **Fix (option 1, the codebase's existing Constant idiom — cf. gap `h_gap`, contact pressure):** for cracking materials only, `lmbda`/`G` are stored as mutable `dolfinx.fem.Constant`s in `spine.load_materials`; `cracking_model.update_cracking` updates them in place via `.value` (float-assign fallback); the writer's compiled Expression references the live Constant, so per-step stiffness propagates with no recompile. Non-cracking materials keep plain floats → byte-for-byte unchanged. `creep_model.py` numpy predictor reads `G` via `getattr(.,"value",.)` (defensive for a future creep+cracking material). **Scope:** only the *output* stress-derived fields (`Stress`, `VonMises`, `Hydrostatic`, `StrainEnergyDensity`) were wrong, and only for cracking runs — the solve, displacement, thermal, gap, contact, and burnup were always correct, so the `pwr_rod_2D` gold (history.csv-based) needs no re-blessing. **Verified:** cracking-off `coaxial_contact` unchanged (3.5% Lamé, pellet σ_rr = −74.5 MPa uniform compressive); cracking-on short pwr_rod now gives σ_rr compressive throughout, −3.1 MPa at the surface (≈ −2 MPa BC), soft-fuel magnitude. **Consequence note:** the earlier "−6.5 GPa fuel rim" in the swelling discussion was largely this writer bug, not the swelling physics (swelling does add real rim stress, but bounded). Re-run the full `pwr_rod_2D` (swelling back on) to regenerate correct stress figures for the paper.

<a id="code-feature-8"></a>
- [ ] **CODE-FEATURE-8** — *Added 2026-06-13.* **Temperature-dependent elastic constants E(T), ν(T).** Both UO2 and Zircaloy soften appreciably with temperature (Zry: ~99 GPa at RT → ~80 GPa at 600 K, a ~20 % effect on PCMI stress; UO2 likewise), so E and ν should be optional symbolic cards exactly like the k(T) hook (`materials/fuel_thermal.py`). **How to add it correctly** (mirrors the k(T) plumbing): (i) in `spine.load_materials`, detect `mat["E"]`/`mat["nu"]` given as `"module.func"` strings, resolve to `_E_func`/`_nu_func`, and **defer** the scalar `lmbda`/`G`/`bulk_modulus` computation (today line ~104 does `float(mat["E"])`, which crashes on a string — guard it); (ii) in `spine.initialize_fields`, right beside the `mat["k"]=k_func(self.T)` block, build `mat["lmbda"]`/`["G"]`/`["bulk_modulus"]` as **UFL expressions in the live `self.T` Function** from `E_func(self.T)`/`nu_func(self.T)`. Then `sigma_mech`/`sigma_th` pick them up by reference, the mechanical solve lags T (→ linear/Picard via the staggered loop, same as k), and — thanks to [CODE-P1-15](#code-p1-15) — the writer output is correct *for free* (it now reads `lmbda`/`G` by reference, not as baked floats). **Three friction points to resolve before it composes with the rod's other physics:** (a) **cracking** — `cracking_model.update_cracking` currently stores `lmbda`/`G` as scalar `Constant`s and rescales with scalar arithmetic (`E_iso = f(ν)ⁿ·E_virgin`, a float); a UFL `E(T)` collides with that. To have *both* on the fuel, refactor cracking so the softening is a scalar **factor** `Constant` multiplying the T-dependent base: `G = crack_factor · G_base(self.T)` — a cleaner unification of the two liveness mechanisms; (b) **creep** — the numpy predictor (`creep_model.py:208`, `float(material["G"])`) breaks on a UFL `G(T)`; it needs `G(T)` interpolated pointwise onto the predictor's DG0 cells (G now varies in space through T); (c) **scalar float(E) sites** — the Gc↔σ_c conversion in `load_materials` (damage materials only) and cracking's `E_virgin` capture assume a scalar E. **Staged plan:** clad E(T) with creep off (cleanest, real ~20 % effect, no cracking collision) → clad E(T) with creep (needs (b)) → fuel E(T) (needs (a)). **First validation step (done 2026-06-13):** a constant-valued symbolic E card for the clad (`materials/zircaloy_E.py`, returns 99.3 GPa) verified the resolve/defer/initialize-as-UFL plumbing reproduces the scalar-E baseline; a genuine correlation drops in behind the same hook. *Cards reverted to scalar E 2026-06-13* (a symbolic E makes `G` a UFL expression, which the numpy creep predictor's `float(G)` cannot consume — friction (b) — so it would crash `pwr_rod_2D` once Norton creep is re-enabled); `zircaloy_E.py` + the symbolic-E resolution in `spine.py` stay in place, validated but dormant until a real correlation lands together with the predictor fix.

<a id="code-feature-9"></a>
- [ ] **CODE-FEATURE-9** — *Added 2026-06-14.* **Adaptive time-stepping + staggered-convergence robustness** (late-life PCMI/creep convergence stall). **Symptom:** in the full `regression/pwr_rod_2D` run (creep on, swelling on, rim peaking on, live deformed-gap conductance), late-life steps stall — `pwr_rod` step 21/25 (~1250 d, gap closed, contact ~21 MPa, deep creep) sat in a *flat limit cycle*: the staggered **creep-predictor consistency** metric oscillated `9.616e-3 ↔ 9.537e-3` for 30+ iterations, never dropping below `mechanical.stag_tol = 1e-3`, so the step grinds to `max_iters: 250` and accepts an **unconverged** iterate (then the next steps do the same → the run never finishes cleanly). **Root cause:** the late-life `creep ↔ contact ↔ gap-conductance ↔ T` loop is a tight multi-physics fixed point; the [CODE-P1-15 follow-on] **live deformed-gap conductance** (gap_model: `h_gap = k_gas / live_gap`) added a feedback path that tightened it. The binding constraint is the creep-predictor channel (the numpy radial-return predictor Δγ₀ feeding the staggered convergence test), and at large late-life dt the joint `(u, Δγ₀, gap, p_contact)` iteration loses contraction (multiplier ≈ −1, period-2). **What was tried (2026-06-14, on a fast reproduction — coarse late-life grid + `max_iters 60`):** (i) **finer dt** (≈59 d/step) — did *not* fix it (still stalled ~4.4e-3); (ii) **more damping** (gap `relax 0.3`, **Aitken off**, `relax_u 0.2`) — *worse* (~5e-2; turning Aitken off hurt). So no single relax/dt knob is a robust fix — this needs a controller, not hand-tuning. **The plan:**
  1. **Convergence-driven adaptive dt with step rejection (core).** Classify each step's staggered outcome: converged in few iters (<~15) → grow dt (×~1.3); converged slowly → keep; hit `max_iters` without converging → **reject the step, halve dt, retry from the start-of-step state.** Replaces the fixed per-segment `n_steps` grid in `generate_power_history` with a controller. A stalled step is rolled back and retried smaller instead of limping on unconverged.
  2. **Checkpoint/restore of all step-state (the one architectural prerequisite).** Rejection requires restoring *every* stateful field to step-start before retry: `T, u, burnup, creep ε_cr (DG0), contact._last_gap, gap._h_gap_prev`, and (when coupled) the **SCIANTIX state arrays**. Make every stateful component checkpointable — a clean invariant **shared with the SCIANTIX coupling** ([CODE-FEATURE-4](#code-feature-4)), which also needs rollback on a rejected step; design the two together.
  3. **Physics-driven dt limiters (proactive).** Cap per-step Δburnup / Δgap / Δcreep-strain / ΔLHR and force small dt through stiff events (power ramps, **gap closure**) — the standard fuel-code practice (TRANSURANUS/FRAPCON/BISON), pre-empting the stall rather than only reacting.
  4. **Creep-predictor under-relaxation (targeted).** The predictor consistency channel may want its *own* under-relaxation (independent of the Aitken-on-u controller) so it can't sustain a period-2 cycle; investigate as a cheaper partial mitigation alongside the controller.
  5. *(later)* **error-driven control** — embedded local-truncation-error estimate (BE vs two half-steps) targeting a temporal tolerance; higher fidelity, deferred.
  **Interim mitigation (for runs needed before the controller exists):** keep `gap_conductance.relax: 0.5` and Aitken on; use a **finer fixed `n_steps` grid through the late-life segment** (the original `[8,8,8]` ≈ 44 d/step reached step 21 before stalling — avoid the ≥140 d/step jumps); keep `max_iters` high and **check the log for stalled (capped) steps** before trusting/`re-blessing` a late-life gold. **Owner:** Zullo (solver-core; thesis-clean once the checkpoint hook exists).
...
- **[CASES-P0-1](#cases-p0-1)**, **[CASES-P0-2](#cases-p0-2)**, **[CASES-P0-3](#cases-p0-3)** all showed the same file-absence-fallback design issue in the old `non-regression.sh`. *Resolved structurally on 2026-06-11:* the discovery-based `non-regression_local.sh` has no silent fallback — gold-less cases are simply not suite members, and a member whose run produces no `non-regression.json` is a loud FAIL.
