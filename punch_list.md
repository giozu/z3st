...
- [ ] **CODE-P2-8** — *Further dead-function audit (lower-confidence — needs per-function verification before deletion).* Candidate unused module-level functions found by a usage sweep, deliberately NOT auto-deleted because they sit inside live modules and/or may be intentional debug/plot aids: (i) `core/diagnostic.py` matrix-debug helpers (`analyze_matrix`, `assemble_and_analyze_matrix`, `check_fixed_dofs`, `check_mechanical_constraints`, `debug_dirichlet_mechanical`, `dense_condition`, `estimate_condition_number_sparse`, `is_symmetric`) — the module's `log` logger is live, these debug fns appear unused; (ii) `utils/output.py` — module is live (the notebook uses `lame_solutions`) but several functions (`export_xdmf`, `plot_residuals`, `plot_scalar`, `plot_radial_displacement`, `thermal_shield_slab_analytical_*`, possibly some `plot3d_*`/`plot1d_*`) look unused — verify against `interactive_gui.ipynb` first; (iii) `utils/utils_plot.py` orphans (`plot_field_along_r_xy`, `plot_field_along_x`, `plot_sigma_cyl`, `plot_sigma_principal`, `rescale_axis`); (iv) `utils/utils_load.py` orphans (`clear_log_file`, `get_ksp_reason_name`, `read_power_history`). Also: the thin `extract_displacement`/`extract_temperature`/`extract_stress` wrappers in `utils_extract_vtu.py` are used (3/6/5 call sites) but overlap with `extract_field` — could be collapsed in the same pass. Each requires a notebook + standalone-script check before removal.
