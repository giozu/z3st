#!/usr/bin/env python3
# --.. ..- .-.. .-.. --- Z3ST convergence plot --.. ..- .-.. .-.. ---
"""
Z3ST case: convergence plot

This utility script parses a Z3ST log file and extracts the relative changes
(e.g., ||Δu||/||u| and ||ΔT||/||T||) during the staggered thermo-mechanical
iterations. The results are plotted in logarithmic scale to visualize
solver convergence trends.

Example
-------
$ python3 ../../utils/plot_convergence.py output_cyl.log
$ python3 ../../utils/plot_convergence.py

The script expects a Z3ST log file (default: 'log.z3st') and
saves the resulting figure as 'convergence.png'.
"""

import re
import sys
import os
import matplotlib.pyplot as plt
import yaml

def parse_convergence_log(logfile: str = "log.z3st"):
    """
    Parse staggering iteration data from a Z3ST output log.
    """
    if not os.path.exists(logfile):
        raise FileNotFoundError(f"File '{logfile}' not found.")

    with open(logfile, "r") as f:
        text = f.read()

    pattern = re.compile(
        r"--- Staggering iteration\s+(\d+)/\d+\s+---"
        r"(?:.*?\|\|ΔT\|\|\s*/\s*\|\|T\|\|\s*=\s*([0-9.eE+-]+))?"
        r"(?:.*?\|\|Δu\|\|\s*/\s*\|\|u\|\|\s*=\s*([0-9.eE+-]+))?"
        r"(?:.*?\|\|ΔD\|\|\s*/\s*\|\|D\|\|\s*=\s*([0-9.eE+-]+))?",
        re.DOTALL,
    )

    iterations, dT_values, du_values, dD_values = [], [], [], []

    for match in pattern.finditer(text):
        it, dT, du, dD = match.groups()
        iterations.append(int(it))
        dT_values.append(float(dT) if dT else None)
        du_values.append(float(du) if du else None)
        dD_values.append(float(dD) if dD else None)

    if not iterations:
        raise RuntimeError(f"No iteration data found in '{logfile}'.")

    return iterations, dT_values, du_values, dD_values

def plot_convergence(iterations, dT_values, du_values, dD_values, save_path="convergence.png"):
    """Plot the convergence residuals for T, u, and D."""
    plt.figure(figsize=(8, 5))

    # Plot residuals
    if any(v is not None for v in du_values):
        plt.semilogy(iterations, du_values, "o-", label=r"residuals(u)", alpha=0.8)
    if any(v is not None for v in dT_values):
        plt.semilogy(iterations, dT_values, "s--", label=r"residuals(T)", alpha=0.8)
    if any(v is not None for v in dD_values):
        plt.semilogy(iterations, dD_values, "d:", label=r"residuals(D)", color="crimson", lw=1.5)

    # Load tolerances from input.yaml
    if os.path.exists("input.yaml"):
        with open("input.yaml", "r") as f:
            config = yaml.safe_load(f)
        
        mech_cfg = config.get("mechanical", {})
        th_cfg = config.get("thermal", {})
        dmg_cfg = config.get("damage", {})

        # Plot tolerances as horizontal lines
        if any(v is not None for v in du_values):
            plt.axhline(float(mech_cfg.get("stag_tol", 1e-8)), lw=1, color="blue", ls="--", label="tol u")
        if any(v is not None for v in dT_values):
            plt.axhline(float(th_cfg.get("stag_tol", 1e-8)), lw=1, color="green", ls="--", label="tol T")
        if any(v is not None for v in dD_values):
            plt.axhline(float(dmg_cfg.get("stag_tol", 1e-4)), lw=1.2, color="red", ls="-", label="tol D")

    plt.xlabel("Staggering iteration")
    plt.ylabel("Relative Residuals")
    plt.title("Z3ST Staggered Solver Convergence")
    plt.grid(True, which="both", ls=":")
    plt.legend(fontsize='small', frameon=True)
    plt.tight_layout()
    plt.savefig(save_path, dpi=200)

    print(f"Plot saved as '{save_path}'")

def main():
    logfile = sys.argv[1] if len(sys.argv) > 1 else "log.z3st"
    try:
        print(f"Reading log file: {logfile}")
        iterations, dT_values, du_values, dD_values = parse_convergence_log(logfile)
        plot_convergence(iterations, dT_values, du_values, dD_values)
    except Exception as e:
        print(f"Error: {e}")

if __name__ == "__main__":
    main()