...
def setup_function_spaces(self, config):
    # Charger le maillage
    self.mesh = self._load_mesh(config["mesh_path"])

    # Charger les tags pour les sous-domaines
    self.tags = self._load_tags(config["mesh_path"])

    # Définir les sous-domaines (steel_in et steel_o)
    self.subdomains = {
        "steel_in": self.tags.find(config["geometry"]["labels"]["steel_in"]),
        "steel_o": self.tags.find(config["geometry"]["labels"]["steel_o"]),
    }

    # Créer les espaces fonctionnels (restreints aux sous-domaines)
    self.V_m = {
        name: fem.VectorFunctionSpace(
            self.mesh,
            ("CG", 1),
            restricted=tag
        )
...
def _load_tags(self, mesh_path):
    """
    Charge les tags physiques depuis le fichier .msh.
    """
    import meshio
    msh = meshio.read(mesh_path)

    # Extraire les tags physiques (cell_data)
    tags = {}
    for key in msh.cell_data_dict:
        if key.startswith("gmsh:physical:"):
            tag_name = key.split(":")[-1]
            tags[tag_name] = msh.cell_data_dict[key]
