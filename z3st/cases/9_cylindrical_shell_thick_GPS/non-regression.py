...
"""
Z3ST case: 9_cylindrical_shell_thick_GPS
...
strain_rr_ref = lambda r: (1 + nu) / E * (A * (1 - 2*nu) - B / r**2) - nu * eps_zz_GPS
strain_tt_ref = lambda r: (1 + nu) / E * (A * (1 - 2*nu) + B / r**2) - nu * eps_zz_GPS
...
strain_zz_ana_L = np.ones_like(r_s) * eps_zz_GPS # generalized plane strain
