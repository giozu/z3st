...
"""
Z3ST case: 13_annular_cylinder

non-regression script
-----------------------
Analytical solution for radial temperature in a 2D
...
LHR = 5.0e2                                 # W/m        linear heat generation
...
def analytic_T(r):
    ...
    return To + LHR / (4 * k * np.pi) * (
        (Ro**2 - r**2) / (Ro**2 - Ri**2) + 2 * Ri**2 * np.log(r / Ro) / (Ro**2 - Ri**2)
    )
...
# Stress
x_S, z_S, _, S_all = extract_field(VTU_FILE, field_name="Stress_volume (cells)")
...
plt.title(rf"$Tmax$ = {np.max(T)-273.15:.0f}°C, $R_i/t$ = {slenderness:.2f}, $\sigma_T$ = {max_sigma_T*1e-6:.1f} MPa", pad=15, fontsize=14)
plt.tight_layout()
