...
# Sweep parameters
BA_LIST = [1.02, 1.04, 1.06, 1.08, 1.10, 1.13, 1.16, 1.20]
MUA_LIST = [5, 10, 20, 50, 100]
...
for ba in BA_LIST:
    for mua in MUA_LIST:
        ...
        # adaptive radial refinement: resolve the inner-wall gamma boundary
        # layer (~1/mu) so the peak hoop stress is mesh-converged. Worse for
        # high mu*a / thick b/a, so scale nx with mua*(ba-1).
        nx_ref = int(min(401, max(101, 12 * mua * (ba - 1) + 80)))
        txt = re.sub(r"nx\s*=\s*\d+;", f"nx = {nx_ref};", txt)
