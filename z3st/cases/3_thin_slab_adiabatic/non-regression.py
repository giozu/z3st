...
---------------------
Steady-state slab (Dirichlet-Neumann).
...
y_target, z_target, mask_tol = Ly / 2, Lz / 2, Lx / 1  # m, m, m (exrtaction line and tolerance)
...
plt.title(rf"$T_i$ = {Ti-273.15:.0f}°C, $Tmax$ = {np.max(T)-273.15:.0f}°C, $\sigma_T$ = {max_sigma_T*1e-6:.1f} MPa", pad=15, fontsize=14)
plt.tight_layout()
