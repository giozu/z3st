...
interference-fit pressure, on an in-plane r-theta quarter-disk section
(regime: 2d, plane strain). The inner disk is heated uniformly, so its free
...
plt.title("Contact pressure verification: Z3ST vs Lame (r-theta disk, uniform ΔT)")
...
def plot_stress_profiles():
    ...
    ax.set_title(f"Radial / hoop stress vs Lame (p = {p:.1f} MPa, r-theta disk)")
