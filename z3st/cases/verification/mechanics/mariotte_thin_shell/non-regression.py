...
ax1.plot(r_s, sigma_rr_ana_M * Pa_to_MPa, "-", color="#D55E00", label=r"Ana. $\sigma_{rr}$ (Radial)", linewidth=1.5)
ax1.plot(
    r_s,
    sigma_tt * Pa_to_MPa,
    "o", color="#009E73",
...
ax1.plot(r_s, sigma_zz_ana_M * Pa_to_MPa, "-", color="#0072B2", label=r"Ana. $\sigma_{zz}$ (Axial)", linewidth=1.5)
...
ax1.plot(r_s, epsilon_rr_ana_M, "-", color="#D55E00", label=r"Ana. $\varepsilon_{rr}$ (Radial)", linewidth=1.5)
ax1.plot(
    r_s,
    epsilon_tt,
    "o", color="#009E73",
...
ax1.plot(
    r_s, epsilon_tt_ana_M, "-", color="#009E73", label=r"Ana. $\varepsilon_{\theta\theta}$ (Hoop)", linewidth=1.5
)
ax1.plot(r_s, epsilon_zz, "o", color="#0072B2", label=r"Num. $\varepsilon_{zz}$ (Axial)", markersize=4, alpha=0.6)
ax1.plot(r_s, epsilon_zz_ana_M, "-", color="#0072B2", label=r"Ana. $\varepsilon_{zz}$ (Axial)", linewidth=1.5)
...
errors = {
    ...
    "L2_error_epsilon_rr": error_metric(err_eps_rr),
    "L2_error_epsilon_tt": error_metric(err_eps_tt),
    "L2_error_epsilon_zz": error_metric(err_eps_zz),
}
