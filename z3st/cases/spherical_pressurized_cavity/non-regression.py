...
from utils_extract_vtu import list_fields, extract_spherical_stresses
...
Pi = 1.0e+6 # internal pressure (Pa)
...
r = r_all[mask]
sigma_rr_num = srr_all[mask]; sigma_tt_num = stt_all[mask]; sigma_pp_num = spp_all[mask]
...
sigma_vm_num = np.sqrt(((sigma_rr_num - sigma_tt_num)**2 + (sigma_tt_num - sigma_pp_num)**2 + (sigma_pp_num - sigma_rr_num)**2) / 2.0)
...
errors = {
    "sigma_rr": {
        ...
        "numerical": np.mean(sigma_rr_num)
        },
    "sigma_tt": {
        "rel_error": float(err_tt),
        "numerical": np.mean(sigma_tt_num)
        },
    "sigma_vm": {
        "rel_error": float(err_vm),
        "numerical": np.mean(sigma_vm_num)
        },
}
